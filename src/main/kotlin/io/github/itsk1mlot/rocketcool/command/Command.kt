...
class Command: CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>?): Boolean {
        if (sender.hasPermission("firework.command")) {
            if (args?.isEmpty() == false) {
                if (args[0] == "reload") {
                    ...
                    sender.sendMessage(Component.text("§6[FireworkCooldown] §c이해할 수 없는 인수입니다. /firework [reload/toggle] 로 사용해주세요."))
                }
            } else {
                sender.sendMessage(Component.text("§6[FireworkCooldown] §c/firework [reload/toggle] 로 사용해주세요."))
                return true
            }
        } else {
            sender.sendMessage(Component.text("§6[FireworkCooldown] §c명령어를 사용하기 위한 권한이 없습니다."))
            return true
        }
        return false
    }
